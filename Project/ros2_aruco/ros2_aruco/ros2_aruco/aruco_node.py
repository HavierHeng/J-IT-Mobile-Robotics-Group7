...
class ArucoNode(rclpy.node.Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter("camera_info_topic", "/zed/zed_node/rgb_raw/camera_info")
        ...
        # Rate limit pose publishing
        self.last_pose_publish_time = self.get_clock().now()
        self.pose_publish_interval = 0.1  # seconds (10 Hz)
    ...
    def image_callback(self, img_msg):
        ...
        if marker_ids is not None:
            ...
            if (now - self.last_pose_publish_time).nanoseconds * 1e-9 > self.pose_publish_interval:
                self.poses_pub.publish(pose_array)
                self.get_logger().info(f"Detected {len(pose_array.poses)} marker pose(s) via /aruco_poses. IDs: {markers.marker_ids}")
